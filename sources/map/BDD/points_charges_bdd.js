var feature_group_pdc = L.featureGroup({}); //.addTo(map_5c3862ba13c7e615013e758f79b1f9bb);
...
function afficherPopupPointsDeCharges(e) {
  ...
  var types_prises = [];
  var puissanceList = [];
  for (var i = 0; i < e.types_prises.length; i++) {
    types_prises.push(e.types_prises[i]["nom"]);
  }
  for (var i = 0; i < e.puissances.length; i++) {
    puissanceList.push(e.puissances[i]["puissance"]);
  }
  var prises = "Types de prises : " + types_prises.join(" - ");
  var puissances = "Puissances : " + puissanceList.join(" - ");

  var popup =
    nom + "<br>" + infos + "<br>" + prises + "<br>" + puissances + "<br>";
  popup +=
    "<button id='pdc' onclick='afficherIsochronePieton(" +
    e.latitude +
    "," +
    e.longitude +
    ")'>Afficher l'isochrone pieton</button>";
  popup +=
    "<button id='pdc' onclick='afficherIsochroneVelo(" +
    e.latitude +
    "," +
    e.longitude +
...
function isInTab(val, liste_pdc) {
  ...
  $.each(liste_pdc, function (key, val2) {
    if (
      val2.puissance == val.puissance_nominale &&
      val2.prise_ef == val.prise_type_ef &&
      val2.prise_2 == val.prise_type_2 &&
      val2.prise_combo == val.prise_type_combo &&
      val2.prise_chademo == val.prise_type_chademo &&
      val2.prise_3c == val.prise_type_3c &&
      val2.prise_autre == val.prise_type_autre &&
      val2.gratuit == val.gratuit
    ) {
      res = true;
    }
  });
...
function afficherInfospdc() {
  ...
  popup +=
    "<p>Nom commercial du reseau : " + pdc.properties.nom_enseigne + "</p>";
  popup += "<p>Adresse : " + pdc.properties.adresse + "</p>";
  if (pdc.properties.deux_roues) {
    popup +=
      '<p>Accepte les deux roues : <span style="color:green;">Oui</span></p>';
  } else {
    popup +=
      '<p>Accepte les deux roues : <span style="color:red;">Non</span></p>';
  }
  popup +=
    "<p>Date de mise en service : " + pdc.properties.mise_en_service + "</p>";
  popup +=
    "<p>Date de dernière modification de la base de donnée : " +
    pdc.properties.mdate +
    "</p>";
  popup += "</div>";
...
function addPopuppdc(pdc, liste_pdc_type) {
  ...
  $.each(liste_pdc_type.features, function (key, val) {
    if (val.properties.rs_st_irve_sta_p == st_irve_sta_p) {
      if (!isInTab(val.properties, liste_pdc)) {
        liste_pdc.push({
          puissance: val.properties.puissance_nominale,
          prise_ef: val.properties.prise_type_ef,
          prise_2: val.properties.prise_type_2,
          prise_combo: val.properties.prise_type_combo,
          prise_chademo: val.properties.prise_type_chademo,
          prise_3c: val.properties.prise_type_3c,
          prise_autre: val.properties.prise_type_autre,
          gratuit: val.properties.gratuit,
        });
      }
    }
  });
  ...
  popup +=
    "<h3> Point de recharge electrique : " + pdc.properties.libelle + "</h3>";
  popup += "<p>Opérateur : " + pdc.properties.nom_operateur + "</p>";
  popup +=
    "<p>Nombre de places de stationnement : " +
    pdc.properties.nb_stationnement +
    "</p>";
  ...
  popup +=
    '<button class="btn btn-primary" onclick="afficherInfospdc()">Voir les infos détaillées</button>';
  popup +=
    "<button id='pdc' onclick='afficherIsochronePieton(" +
    pdc.geometry.coordinates[1] +
    "," +
    pdc.geometry.coordinates[0] +
    ")'>Afficher l'isochrone pieton</button>";
  popup +=
    "<button id='pdc' onclick='afficherIsochroneVelo(" +
    pdc.geometry.coordinates[1] +
...
function updateBddStationsPointsDeCharges() {
  ...
  //         // pour chaque pdc
  //         pt_electriques++;
  //         var marker = L.marker([pdc.latitude, pdc.longitude], {
  //           icon: pdcIcon,
  //         }).addTo(feature_group_pdc); // création du marqueur
  ...
  $.ajax({
    url: baseURL + "points_recharge/",
    dataType: "json",
    cache: false,
    success: function (data) {
      {
        $.each(data.data, function (key, pdc) {
          try {
            // pour chaque pdc
            pt_electriques++;
            var length = pdc.coordonnees.length;
            if (length == 1) {
              // si le pdc n'a qu'une seule coordonnée
              var marker = L.marker(
                [pdc.coordonnees[0].latitude, pdc.coordonnees[0].longitude],
                { icon: pdcIcon }
              ).addTo(feature_group_pdc); // création du marqueur
              marker.bindPopup(afficherPopupPointsDeCharges(pdc)); // ajout du popup
            } else if (length > 1) {
              // si le pdc a plusieurs coordonnées
              var latlngs = []; // création d'un tableau vide
              var sumLat = 0;
              var sumLng = 0;
              pdc.coordonnees.forEach(function (coordonnee) {
              ...
              L.marker([sumLat / length, sumLng / length], {
                ...
                .addTo(feature_group_pdc)
                .bindPopup(afficherPopupPointsDeCharges(pdc));
              var polygon = L.polygon(latlngs).addTo(feature_group_pdc); // création du polygone
              polygon.setStyle({ color: "blue" }); // changement de la couleur du polygone
              polygon.bindPopup(afficherPopupPointsDeCharges(pdc)); // ajout du popup
              affichage();
            }
          } catch (e) {
            console.log(e);
      ...
      $.ajax({
        ...
        success: function (data_2) {
          $.each(data.features, function (key, val) {
            ...
            pt_electriques++;
            L.marker([latitude, longitude], { icon: pdcIcon })
              .bindPopup(addPopuppdc(val, data_2))
              .addTo(feature_group_pdc);
            affichage();
          });
          checkSumInitialLoaging++;
