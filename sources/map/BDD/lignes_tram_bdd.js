function updateBddLignesTram(){
    // suppression des marqueurs existants de la carte
    feature_group_ligne_tram.eachLayer(function (layer) {
        if (layer instanceof L.Marker || layer instanceof L.Polyline) {
            feature_group_ligne_tram.removeLayer(layer);
        }
    });
        ...
        data.forEach(function(ligne) { // pour chaque ligne
            if (ligne.type == "Tramway") {
                if (ligne.coordonnees.length == 1) {// si le ligne n'a qu'une seule coordonnée
                    ...
                    feature_group_ligne_tram.addLayer(marker); // ajout du marqueur au groupe de marqueurs
                }
