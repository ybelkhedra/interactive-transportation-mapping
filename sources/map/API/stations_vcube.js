var feature_group_vcube = L.featureGroup({}); //.addTo(map_5c3862ba13c7e615013e758f79b1f9bb);

var vcub_Icon = L.icon({
  iconUrl: "./sources/icons/vcub.png",
...
function addPopupvcube(vcube) {
  ...
  if (vcube.properties.etat == "CONNECTEE") {
    popup +=
      '<p>Etat : <span style="color:green;">Station connectée</span></p>';
  } else if (vcube.properties.etat == "DECONNECTEE") {
    popup +=
      '<p>Etat : <span style="color:red;">Station déconnectée</span></p>';
  } else if (vcube.properties.etat == "MAINTENANCE") {
    popup +=
      '<p>Etat : <span style="color:orange;">Station en maintenance</span></p>';
  }
  popup += "<p>Nombre de places libres : " + vcube.properties.nbplaces + "</p>";
  popup +=
    "<p>Nombre total de vélos disponibles : " +
    vcube.properties.nbvelos +
    "</p>";
  popup +=
    "<p>Nombre de vélos électriques disponibles : " +
    vcube.properties.nbelec +
    "</p>";
  popup +=
    "<p>Nombre de vélos classiques disponibles : " +
    vcube.properties.nbclassiq +
    "</p>";
  popup +=
    "<button id='vcub' onclick='afficherIsochronePieton(" +
    vcube.geometry.coordinates[1] +
    "," +
    vcube.geometry.coordinates[0] +
    ")'>Afficher l'isochrone pieton</button>";
  popup +=
    "<button id='vcub' onclick='afficherIsochroneVelo(" +
    vcube.geometry.coordinates[1] +
    "," +
    vcube.geometry.coordinates[0] +
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(data.features, function (key, val) {
        ...
        L.marker([latitude, longitude], { icon: vcub_Icon })
          .bindPopup(addPopupvcube(val))
          .addTo(feature_group_vcube);
        affichage();
      });
